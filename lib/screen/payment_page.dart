...
class _PaymentPageState extends State<PaymentPage> {
  ...
  void didUpdateWidget(covariant PaymentPage oldWidget) {
    ...
    // ກວດສອບວ່າ:
    // 1. paymentMethods ໃໝ່ ມີຂໍ້ມູນ (ບໍ່ວ່າງ)
    // 2. paymentMethods ເກົ່າ ບໍ່ມີຂໍ້ມູນ (ວ່າງ)
    // 3. _selectedPaymentCode ຍັງບໍ່ໄດ້ຖືກເລືອກ (ຍັງເປັນ null)
    // (ນີ້ໝາຍຄວາມວ່າ paymentMethods ຫາກໍໂຫຼດມາສຳເລັດ)
    if (widget.paymentMethods.isNotEmpty &&
        oldWidget.paymentMethods.isEmpty &&
        _selectedPaymentCode == null) {
      // ສັ່ງ setState ເພື່ອເລືອກ payment method ທຳອິດເປັນຄ່າເລີ່ມຕົ້ນ
      setState(() {
        _selectedPaymentCode = widget.paymentMethods.first.code;
      });
    }
  ...
  // 🎯 [FIX 2] ລົບ _buildVisitorForm() ອອກໄປເລີຍ
